...
class WatcherBot: 
    ...
    def __init__(self, manager:ShowManager):
        ...
        self.cmd_actions = [
                ...
                ('possible_shows $people',  'get list of possible shows to watch (separate people names by commas)',          self.get_possible),
                ...
                ('add_show',        'add new show',                                 self.add_show),
    ...
    async def get_possible(self,update,context) -> None:
        ...
        watchers : list[str] = watcher_str.split(', ')
        ...
        show_names : list[str] = list(map(lambda x: x.name,shows))

        ret_msg : str = 'Possible shows to watch with %s:\n\t %s' % (', '.join(watcher_names),', '.join(show_names))
